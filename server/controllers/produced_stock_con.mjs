import producedStock from "../models/produced_stock.mjs";
import mongoose from "mongoose";

//add product
export const addStock = async(req, res) => {
    try{
        const { product_id, product_name, produced_quantity, } = req.body;

        const newStock = new producedStock({
            product_id, product_name, produced_quantity,
        });
...
export const showStock = async(req, res) => {
    try {
        const stock = await producedStock.find();

        if(stock.length === 0){
            return res.status(404).json({ message: "No produced Stock"});
        }
...
export const deleteProduct = async (req, res) => {
    try {
        ...
        const deletedProduct = await producedStock.findByIdAndRemove(productid);
...
export const updateProduct = async (req, res) => {
    try {
        ...
        const updatedProduct = await producedStock.findByIdAndUpdate(productid, update, { new: true });
